...
func (r *userRepository) GetUserByID(ctx context.Context, id int) (entity.User, error) {
	...
	if err := r.db.WithContext(ctx).Select("id", "name", "email", "mobilephone", "created_at", "updated_at").First(&user, id).Error; err != nil {
...
func (r *userRepository) UpdateUser(ctx context.Context, id int, user entity.User) (entity.User, error) {
	...
	if err := r.db.WithContext(ctx).Select("id", "name", "email", "mobilephone", "created_at", "updated_at").First(&existingUser, id).Error; err != nil {
...
func (r *userRepository) GetAllUsers(ctx context.Context) ([]entity.User, error) {
	...
	if err := r.db.WithContext(ctx).Select("id", "name", "email", "mobilephone", "created_at", "updated_at").Find(&users).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return users, nil
		}
